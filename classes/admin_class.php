...
class Admin_Class
{
	...
	{
		// data insert   
		$task_year = $this->test_form_input_data($data['task_year']);
		...
		$product = $this->test_form_input_data($data['select_product']);
		...
		try {
			$num = 1;
			$ticketNo = "SSI"."/".$product."/".$task_year;
			$add_task = $this->db->prepare("INSERT INTO task_info (t_num,t_year,t_title, t_description, t_start_time, t_end_time,select_product, t_user_id) VALUES (:t,:k,:x, :y, :z, :a,:s,:b)");
 
			$add_task->bindparam(':k', $task_year);
			...
			$add_task->bindparam(':s', $product);
			$add_task->bindparam(':b', $assign_to);
			$add_task->bindparam(':t', $ticketNo);
